...
pub fn commit_bid(
    ...
    if bidder_kind == BidderKind::User {
        ...
        let bidder_state = UserState::try_deserialize(&mut &bidder_data[..]).map_err(|e| {
            msg!("UserState deserialize error: {:?}", e);
            error!(SableError::InvalidRecipientAccounts)
        })?;
        require!(bidder_state.owner == signer, SableError::NotAuthorized);
        let bidder_owner = bidder_state.owner;
        drop(bidder_data);

        debit_bidder_user_balance(
            &ctx.accounts.bidder_balance,
            bidder_owner,
            mint_key,
            deposit,
        )?;
        ...
        let (expected_counters, _) = Pubkey::find_program_address(
            &[crate::AGENT_COUNTERS_SEED.as_bytes(), bidder_key.as_ref()],
            ctx.program_id,
        );
        require!(
    ...
    task.bid_count = task.bid_count.checked_add(1).ok_or(SableError::Overflow)?;
